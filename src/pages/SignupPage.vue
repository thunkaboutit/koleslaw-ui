...
onMounted(async () => {
  ...
  if (route.query.error) {
    ...
    error.value = typeof msg === 'string' ? msg : 'Sign-up failed. Please try again.'
  }
})

function signupWith(provider: string) {
  ...
  <div class="signup">
    ...
    <div class="signup__card">
      <h1 class="signup__title">
        <RouterLink to="/" class="signup__title-link">Koleslaw</RouterLink>
      </h1>
      <p class="signup__subtitle">Create your account</p>

      <p v-if="error" class="signup__error" role="alert">{{ error }}</p>

      <div class="signup__providers">
        <button
          v-for="provider in providers"
          :key="provider.id"
          class="signup__provider"
          :class="`signup__provider--${provider.id}`"
          :disabled="loading !== null"
          @click="signupWith(provider.id)"
        >
          <span v-if="loading === provider.id" class="signup__spinner" aria-hidden="true"></span>
          <template v-else>
            <!-- Google -->
            <svg
      ...
      <p class="signup__login">
        ...
        <RouterLink to="/login" class="signup__login-link">Sign in</RouterLink>
      </p>
    </div>
  </div>
</template>
...
.signup__provider:disabled {
  opacity: 0.5;
  cursor: not-allowed;
}

.signup__provider:not(:disabled):hover {
  opacity: 0.85;
}

/* Google — white bg, dark text */
.signup__provider--google {
  background: var(--wl-white);
  color: var(--wl-black);
}

/* GitHub — dark bg, light text */
.signup__provider--github {
